...
def quantization_model():
    ...
    weights = r'weights_24/unzip.pt'
    cfg = r'weights_24/yolov3_24cls_opt5_exam.cfg'
    imgsz = image_size
    # Initialize
    device = torch.device('cpu')
    # Initialize model
    model = Darknet(cfg, imgsz[0],conf_thres=confidence, nms_thres=nms)
    # Load weights
    ...
    torch.save(checkpoint, 'weights_24/best_qua.pth')
...
def show_result(img_path, outputs2, size_shape):
    ...
    pred = non_max_suppression(outputs2, number_class, confidence, nms)
    print(pred[0].shape)
...
def val_model():
    qua_weights = r'weights_24/best_qua.pth'
    cfg = r'weights_24/yolov3_24cls_opt5_exam.cfg'
    imgsz = image_size
    # Initialize
...
def show_result_mlu(img_path, outputs2, size_shape):
    ...
    for i in range(numBoxFinal):
        c1 = (int(outputs2[0][64+i*7+3]), int(outputs2[0][64+i*7+4]))
        c2 = (int(outputs2[0][64+i*7+5]), int(outputs2[0][64+i*7+6]))
