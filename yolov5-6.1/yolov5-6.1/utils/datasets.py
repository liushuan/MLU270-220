...
from utils.augmentations import Albumentations, augment_hsv, copy_paste, letterbox, mixup, random_perspective, AddBlur
...
def extract_boxes(path=DATASETS_DIR / 'coco128'):  # from utils.datasets import *; extract_boxes()
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in IMG_FORMATS:
            ...
            im = cv2.imdecode(np.fromfile(str(im_file), dtype=np.uint8), cv2.IMREAD_COLOR)[..., ::-1]
            h, w = im.shape[:2]
