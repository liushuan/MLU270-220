...
def yolov3(pretrained=False, img_size=416, conf_thres=0.8, nms_thres=0.4):
    ...
    Args:
        ...
        img_size: image size
    """
    print("start generate yolov3.")
    cwd = os.path.dirname(os.path.abspath(__file__))
    config_path = os.path.join(cwd, 'config', 'yolov3_29cls_810.cfg')
    print("config_path:", config_path)
    print("model_urls:", model_urls['yolov3'])
    model_path = r'/opt/cambricon/yolov3/weights/yolov3.pth'
    model = Darknet(config_path, img_size, conf_thres, nms_thres).eval()
    if pretrained:
       # model.load_state_dict(model_zoo.load_url(model_urls['yolov3']), strict=False)
