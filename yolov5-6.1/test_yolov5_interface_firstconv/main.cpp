...
void test_exam(){
	...
	EXAMDetect examDetect("./weights/yolov5_220_1_1_first.cambricon");
	
	std::vector<std::string> files, file_names;
	std::string path = "./images";
	util::getFiles(path, files, file_names, ".jpg");
	std::cout<<"file size:"<<files.size()<<std::endl;
	for(int i = 0; i < files.size()*1000; i++){
		cv::Mat img = cv::imread(files[i%2]);
		//std::cout<<"file:"<<files[i]<<std::endl;
		//int pad_top, pad_left, resize_w, resize_h;
		//img = resize_with_crop(img, 640, 640, pad_top, pad_left, resize_w, resize_h);
