...
/*bool tracker::direct_in(Track &t) {
DETECTBOX box1 = t.start_box;
DETECTBOX box2 = t.current_box;
float center_x1 = box1[0] + box1[2] / 2.0;
float center_y1 = box1[1] + box1[3] / 2.0;

float center_x2 = box2[0] + box2[2] / 2.0;
float center_y2 = box2[1] + box2[3] / 2.0;

if (count == 0) {
count++;
dir_x = center_x2 - center_x1;
dir_y = center_y2 - center_y1;
return true;
}
else {
int new_dx = center_x2 - center_x1;
int new_dy = center_y2 - center_y1;
if (dir_x*new_dx + dir_y*new_dy > 0) {
dir_x = beta*dir_x + (1 - beta)*new_dx;
dir_y = beta*dir_y + (1 - beta)*new_dy;
return true;
}
else {
return false;
...
bool tracker::direct_in(Track &t) {
	...
	if (count == 0 && (dir_x != 0 || dir_y != 0)) {
		count = 1;
	}
	else if (count == 0 && dir_x == 0 && dir_y == 0) {
		count = 2;
	}
	int new_dx = center_x2 - center_x1;
	int new_dy = center_y2 - center_y1;
...
void tracker::update(const DETECTIONS &detections, std::vector<Track>&delete_t)
{
	TRACHER_MATCHD res;
	...
	//#endif
	vector<Track, Eigen::aligned_allocator<Track>>::iterator it;
	for (it = tracks.begin(); it != tracks.end();) {
		if ((*it).is_deleted()) {
			if ((*it).last_state == Track::TrackState::Confirmed) {
				if (direct_in((*it))) {
					track_count_in[(*it).classify]++;
				}
				else {
					track_count_out[(*it).classify]++;
				}
				delete_t.push_back(*it);
			}
			it = tracks.erase(it);
		}
		else {
	...
	*/
	vector<int> active_targets;
	vector<TRACKER_DATA, Eigen::aligned_allocator<TRACKER_DATA>> tid_features;
	for (Track& track : tracks) {
		if (track.is_confirmed() == false) continue;
		active_targets.push_back(track.track_id);
...
bool tracker::delete_track(int track_id) {
	vector<Track, Eigen::aligned_allocator<Track>>::iterator it;
...
void tracker::_initiate_track(const DETECTION_ROW &detection)
{
	...
	this->tracks.push_back(Track(mean, covariance, this->_next_idx, this->n_init,
		this->max_age, detection.feature, detection.classify, detection.confidence, detection.tlwh));
	_next_idx += 1;
	_next_idx = _next_idx % max_size;
}
