...
def genaratePsf(length, angle):
    ...
    psf1 = psf1 / psf1.sum()
    return psf1, anchor
def AddBlur(img, start=3, end = 15):
    random_value = random.random()
    level = random.randrange(1, 5)
    if random_value > 0.9:
        return cv2.blur(img, (level * 2 + 1, level * 2 + 1))
    elif random_value >0.75:
        return cv2.GaussianBlur(img, (level * 2 + 1, level * 2 + 1), 0)
        ...
        while True:
            ...
            if angle != 90 and angle != 180 and angle != 270:
                break
        kernel, anchor = genaratePsf(offset, angle)
        return cv2.filter2D(img, -1, kernel, anchor=anchor)
    else:
        return img
