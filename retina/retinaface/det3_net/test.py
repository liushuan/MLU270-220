...
def quanza_model():
    ...
    net = ONET().to(device)
    net.load_state_dict(torch.load("./det3-half.pth", map_location=device), strict=True)
    net.eval()
    print('Finished loading model!')
    #print(net)
    
    input_img = get_image_tensor("imgs/1.jpg")
    ...
    torch.save(net_quantization.state_dict(), 'det3_quantize.pth')
...
def fuse_quanza_model():
    ...
    net_quantization.load_state_dict(torch.load('det3_quantize.pth'))
    print("quantization model load finish")
    
    input_img1 = get_image_tensor("imgs/1.jpg")
    input_img1 = torch.from_numpy(input_img1)
    
    ct.save_as_cambricon("det3.cambricon")
