...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        print("anchors_list:", self.anchors_list, "num_anchors:", self.num_anchors, "self.nc:", self.nc)
        self.img_h = 640
        self.img_w = 640
        self.conf_thres = 0.1
        self.iou_thres = 0.4
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y = x[i].sigmoid()
                y = y.data.cpu().type(torch.FloatTensor)
                if self.inplace:
    ...
    def _make_grid(self, nx=20, ny=20, i=0):
        ...
        if check_version(torch.__version__, '1.10.0'):  # torch>=1.10.0 meshgrid workaround for torch>=0.7 compatibility
            ...
            yv, xv = torch.meshgrid([torch.arange(ny, device='cpu'), torch.arange(nx, device='cpu')])
