...
public:
    ...
    void partial_fit(std::vector<TRACKER_DATA, Eigen::aligned_allocator<TRACKER_DATA>>& tid_feats, std::vector<int>& active_targets);
...
private:
    PTRFUN _metric;
    int budget;
    std::map<int, FEATURESS ,std::less<int>,Eigen::aligned_allocator<std::pair<int, FEATURESS> >> samples;
